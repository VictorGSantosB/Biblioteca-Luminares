...
@section('content')
    <main class="min-h-screen flex justify-center p-10">
        <div class="w-full max-w-6xl">
            <div class="overflow-x-auto">
                @if ($categorias->isEmpty())
                    <p class="text-center text-gray-500 py-4">Não há categorias disponíveis.</p>
                @else
                                ...
                                <tr class="bg-white border-b dark:border-gray-700">
                                    ...
                                    @if (Auth::id() === 1)
                                        <td class="flex gap-5 py-5 px-3">
                                            ...
                                            <form method="get" action="{{ route('categoria.modal.delete') }}">
        ...
        @if (Auth::id() === 1)
            <button onclick="openModal()" class="flutuante round-6">Add</button>
        @endif
    </main>
                ...
                <div class="px-4 py-5 sm:p-6">
                    <form action="{{ route('categoria.update') }}" method="POST"
                        ...
                        <div class="pb-3">
                            ...
                            <input required
                                class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 @error('nome') border-red-500 @enderror"
                                name="nome" id="categoryName" type="text" placeholder="Nome da Categoria"
                                value="{{ old('nome') }}">
                            @error('nome')
